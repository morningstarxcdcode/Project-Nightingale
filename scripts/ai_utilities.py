...
def preprocess_data(data: str) -> str:
    ...
    if not isinstance(data, str):
        raise DataValidationError(
            f"Data must be a string, got {type(data).__name__}", invalid_data=data
        )
...
def evaluate_model(predictions: List[Any], actuals: List[Any]) -> float:
    ...
    if len(predictions) == 0:
        raise DataValidationError(
            "Cannot evaluate with empty lists", invalid_data={"length": 0}
        )
    ...
    try:
        ...
        logger.info(
            f"Model evaluation complete: {correct_predictions}/{len(actuals)} "
...
def calculate_metrics(
    predictions: List[Union[int, float]], actuals: List[Union[int, float]]
) -> Dict[str, float]:
    """
    ...
    try:
        ...
        raise ModelProcessingError(f"Failed to calculate metrics: {str(e)}") from e
